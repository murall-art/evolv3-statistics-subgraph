import { Transfer as TransferEvent } from '../generated/Evolv3/Evolv3'
import {
  ArtistStatistic,
  Artist,
  Artwork,
...
export function handleTransfer(event: TransferEvent): void {
  ...
  if (artist == null) {
    ...
    artist.paintBurned = BigInt.fromI32(0)
    artistStatistic.totalArtists = artistStatistic.totalArtists.plus(
      BigInt.fromI32(1),
    )
  }
  ...
  artistStatistic.totalArtworks = artistStatistic.totalArtworks.plus(
    BigInt.fromI32(1),
  )
  ...
  for (let i = 0; i < logs.length; i++) {
    ...
    if (isPaintBurnLog(log)) {
      ...
      let paintBurned = BigInt.fromByteArray(
        ByteArray.fromHexString(paintBurnedHex).reverse() as ByteArray,
      )
      artwork.paintBurnedRaw = paintBurnedHex
      artwork.paintBurned = paintBurned
      artist.paintBurned = artist.paintBurned.plus(paintBurned)
      paintToken.totalPaintBurned =
        paintToken.totalPaintBurned.plus(paintBurned)
      break
    }
  }
